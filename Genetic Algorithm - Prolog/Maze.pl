...
:- dynamic mazeExit/1, freeSpace/1, mazeSpawn/1, lengthMaze/1, root/1, dist/2.
...
%Maze recursions
clearMaze() :- retractall(mazeExit(_)), retractall(mazeSpawn(_)), retractall(freeSpace(_)), retractall(root(_)), retractall(dist(_, _)).
